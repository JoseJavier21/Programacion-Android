...
import com.estech.retrofitsample.retrofit.Repository
...
class FragmentLista : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            val respuesta = repositorio.getallpersonajes()

            withContext(Dispatchers.Main) {
                if (respuesta.isSuccessful && respuesta.code() == 200) {
                    val mirespuesta = respuesta.body()
                    val info = mirespuesta?.info
                    val listapersonajes = mirespuesta?.results
